...
namespace ImGui
{
    void Hotkey(int id, int* k, const ImVec2& size_arg = ImVec2(0, 0))
    {
        ...
        ;
        if (waitingforkey[id] == false) {
            std::string label = (KeyNames[*(int*)k] + hash);
            if (ImGui::Button(label.c_str(), size_arg))
                waitingforkey[id] = true;
        }

        else if (waitingforkey[id] == true) {
